...
// __dirname and __filename are provided by Node.js in CommonJS modules
    ...
    if (compiler.options.watch) {
      ...
      compiler.hooks.done.tap('watch_tavern_helper', () => {
        ...
        if (compiler.options.plugins.some(plugin => plugin instanceof HtmlWebpackPlugin)) {
          this.io?.emit('message_iframe_updated');
        } else {
          this.io?.emit('script_iframe_updated');
        }
      });
    }
  }
...
class TavernSyncPlugin {
  private child_process: ChildProcess | undefined;
  ...
  apply(compiler: webpack.Compiler) {
    ...
    compiler.hooks.watchRun.tap('watch_tavern_sync', () => {
      if (!this.child_process) {
        this.child_process = spawn('pnpm', ['sync', 'watch', 'all', '-f'], {
          shell: true,
          stdio: ['ignore', 'pipe', 'pipe'],
          cwd: __dirname,
          env: { ...process.env, FORCE_COLOR: '1' },
        });
        this.child_process.stdout?.on('data', (data: Buffer) => {
          console.info(
            data
              .toString()
              .trimEnd()
        ...
        this.child_process.stderr?.on('data', (data: Buffer) => {
          console.error(
            data
              .toString()
              .trimEnd()
...
function parse_configuration(entry: Entry): (_env: any, argv: any) => webpack.Configuration {
  ...
  return (_env, argv) => ({
    ...
    plugins: ([
      ...(entry.html === undefined
        ? [new MiniCssExtractPlugin({ filename: `${script_filepath.name}.css` })]
        : [
      ...
      new WatchTavernHelperPlugin() as unknown as webpack.WebpackPluginInstance,
      new SchemaDumpPlugin() as unknown as webpack.WebpackPluginInstance,
      new TavernSyncPlugin() as unknown as webpack.WebpackPluginInstance,
      new VueLoaderPlugin(),
      unpluginAutoImport({
        dts: true,
        dtsMode: 'overwrite',
    ...
    externals: ({ context, request }, callback) => {
      ...
      if (request === 'vue' || request === 'vue-router') {
        return callback();
      }
